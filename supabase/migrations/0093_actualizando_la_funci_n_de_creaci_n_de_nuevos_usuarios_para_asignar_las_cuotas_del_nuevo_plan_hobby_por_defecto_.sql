...
BEGIN
  ...
  ELSIF new.raw_user_meta_data ->> 'role' = 'admin' THEN
    default_permissions := '{"can_create_server": true, "can_manage_docker_containers": true, "can_manage_cloudflare_domains": false, "can_manage_cloudflare_tunnels": false}';
    v_max_servers := 10;
    v_max_containers := 20;
    v_max_tunnels := 20;
    v_cpu_limit := 2.00;
  ...
  ELSE -- Default for 'user' role (Free "Hobby" Plan)
    default_permissions := '{"can_create_server": true, "can_manage_docker_containers": true, "can_manage_cloudflare_domains": false, "can_manage_cloudflare_tunnels": false}';
    v_max_servers := 1;
    v_max_containers := 1;
    v_max_tunnels := 1;
    v_cpu_limit := 0.50;
    v_memory_limit_mb := 512;
    v_storage_limit_mb := 250; -- 250 MB for user
  END IF;
