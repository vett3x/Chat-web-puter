...
async function runValidationAndFixLoop(server: any, containerId: string, prompt: string, filePlan: FilePlan[], generatedFiles: Map<string, string>, appId: string, userId: string, maxRetries = 3) {
  for (let i = 0; i < maxRetries; i++) {
    ...
    console.warn(`[Provisioning] Validation failed (Attempt ${i + 1}/${maxRetries}). Error: ${stderr}`);
    
    // Simple error parsing: find the first file path mentioned in the error
    ...
    const fixPrompt = `
      La compilación de la aplicación Next.js falló con el siguiente error de TypeScript:
      --- ERROR ---
      ${stderr}
      --- FIN DEL ERROR ---
    ...
    generatedFiles.set(problematicFilePath, fixedContent); // Update our in-memory copy
  }

  throw new Error(`Failed to fix compilation errors after ${maxRetries} attempts.`);
}
...
export async function provisionApp(data: AppProvisioningData) {
  ...
  try {
    ...
    // FASE 2: Planificación y Generación de Código Incremental
    ...
    // FASE 3: Validación y Auto-Corrección
    await runValidationAndFixLoop(server, containerId, prompt, plan.files, generatedFilesContent, appId, userId);
