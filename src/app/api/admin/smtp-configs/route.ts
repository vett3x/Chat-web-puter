...
export async function GET(req: NextRequest) {
  ...
  try {
    const { data, error } = await supabaseAdmin
      .from('smtp_configs')
      .select('id, nickname, host, port, secure, "user", is_active, status, last_tested_at, created_at')
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (config.is_active) {
      await supabaseAdmin.from('smtp_configs').update({ is_active: false }).eq('is_active', true);
    }

    const { error } = await supabaseAdmin.from('smtp_configs').insert(config);
    if (error) throw error;

    return NextResponse.json({ message: 'Configuración SMTP guardada.' }, { status: 201 });
  } catch (error: any) {
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    if (!config.password) delete updateData.password;

    if (config.is_active) {
      await supabaseAdmin.from('smtp_configs').update({ is_active: false }).eq('is_active', true).neq('id', config.id);
    }

    const { error } = await supabaseAdmin.from('smtp_configs').update(updateData).eq('id', config.id);
    if (error) throw error;
...
export async function DELETE(req: NextRequest) {
  ...
  const { error } = await supabaseAdmin.from('smtp_configs').delete().eq('id', id);
