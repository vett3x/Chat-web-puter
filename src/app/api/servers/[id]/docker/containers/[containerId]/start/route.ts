...
export async function POST(
  ...
  try {
    await new Promise<void>((resolve, reject) => {
      conn.on('ready', () => {
        conn.exec(`docker start ${containerId}`, (err, stream) => {
          if (err) {
            conn.end();
          ...
          stream.on('close', (code: number) => {
            ...
            console.error(`STDERR from docker start: ${data.toString()}`);
          });
        });
      }).on('error', (err) => {
    ...
    return NextResponse.json({ message: `Contenedor ${containerId} iniciado.` }, { status: 200 });
  } catch (error: any) {
    console.error(`Error starting Docker container ${containerId} on server ${serverId}:`, error.message);
    return NextResponse.json({ message: `Error al iniciar contenedor Docker: ${error.message}` }, { status: 500 });
  }
}