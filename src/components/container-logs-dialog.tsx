...
export function ContainerLogsDialog({ open, onOpenChange, server, container }: ContainerLogsDialogProps) {
  ...
  const initializeSocket = useCallback(async (term: any) => {
    ...
    try {
      ...
      console.log(`[ContainerLogsDialog] Attempting WebSocket connection to: ${wsUrl}`);

      const ws = new WebSocket(wsUrl);
      wsRef.current = ws;

      ws.onopen = () => {
        term.writeln('\r\n\x1b[32m[CLIENT] Conexión WebSocket establecida. Mostrando logs...\x1b[0m\r\n');
      };
      ...
      console.error('[ContainerLogsDialog] Error during socket initialization:', e);
  ...
  useEffect(() => {
    ...
    const initTerminal = async () => {
      if (terminalRef.current && !termRef.current) {
        ...
        term = new Terminal({
          cursorBlink: false, // No cursor needed for logs
          convertEol: true,
        ...
        term.writeln('\x1b[32m[CLIENT] Terminal de logs inicializada.\x1b[0m');
        
        setTimeout(() => fitAddon.fit(), 10);

        // No term.onData handler, making it read-only
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[80vw] w-[80vw] h-[80vh] flex flex-col p-4">
        <DialogHeader className="px-2 pt-2">
          <DialogTitle>Logs del Contenedor: {container.Names} ({container.ID.substring(0, 12)})</DialogTitle>
          <DialogDescription>
