...
import { Send, Bot, Loader2, Paperclip, KeyRound, Trash2, MessageSquare, Wrench, Folder } from 'lucide-react';
import { toast } from 'sonner';
import { AI_PROVIDERS } from '@/lib/ai-models';
import { useUserApiKeys, AiKeyGroup } from '@/hooks/use-user-api-keys';
...
export function ChatInput({ isLoading, selectedModel, onModelChange, sendMessage, isAppChat = false, onClearChat, chatMode, onChatModeChange }: ChatInputProps) {
  ...
  const { userApiKeys, aiKeyGroups } = useUserApiKeys();
  ...
  const SelectedModelIcon = React.useMemo(() => {
    ...
    return Bot;
  }, [selectedModel, userApiKeys, aiKeyGroups]);
    ...
    <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-background/80 to-transparent pt-8">
      <div className="flex justify-center px-4 pb-4">
        <StarBorder
          ...
          contentClassName="p-2 flex flex-col gap-2 bg-[var(--chat-bubble-background-color)] backdrop-blur-[var(--chat-bubble-blur)] border border-[var(--chat-bubble-border-color)] rounded-lg overflow-hidden" // Añadido overflow-hidden aquí
        >
