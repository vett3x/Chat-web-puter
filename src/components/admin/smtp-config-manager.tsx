...
const configSchema = z.object({
  ...
  port: z.coerce.number().int().min(1, 'El puerto es requerido.'),
  secure: z.boolean(),
...
interface SmtpConfig extends ConfigFormValues {
...
export function SmtpConfigManager() {
  ...
  const form = useForm<ConfigFormValues>({
    ...
    defaultValues: { nickname: '', host: '', port: 587, secure: true, user: '', password: '', is_active: false },
  });
  ...
  const handleAddNew = () => {
    ...
    form.reset({ nickname: '', host: '', port: 587, secure: true, user: '', password: '', is_active: false });
    setIsDialogOpen(true);
  };

  const onSubmit = async (values: ConfigFormValues) => {
    if (editingConfig && !values.password) {
      delete values.password;
    } else if (!editingConfig && !values.password) {
  ...
  const handleDelete = async (id: string) => {
    try {
      const response = await fetch(`/api/admin/smtp-configs?id=${id}`, { method: 'DELETE' });
      const result = await response.json();
      if (!response.ok) throw new Error(result.message);
      toast.success(result.message);
      fetchConfigs();
  ...
  const handleTestConnection = async (id: string) => {
    ...
    try {
      const response = await fetch('/api/admin/smtp-configs/test', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleSetActive = async (id: string) => {
    ...
    try {
      ...
      const result = await response.json();
      if (!response.ok) throw new Error(result.message);
      toast.success(`'${configToActivate.nickname}' ahora es la configuración SMTP activa.`);
      fetchConfigs();
    } catch (err: any) {
  ...
  return (
    <div>
      <div className="flex justify-end mb-4">
        <Button size="sm" onClick={handleAddNew}><PlusCircle className="mr-2 h-4 w-4" /> Añadir Configuración</Button>
      </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>{editingConfig ? 'Editar' : 'Añadir'} Configuración SMTP</DialogTitle></DialogHeader>
          <Form {...form}><form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 py-4">
            <FormField control={form.control} name="nickname" render={({ field }) => (<FormItem><FormLabel>Apodo</FormLabel><FormControl><Input placeholder="Ej: SendGrid, Mailgun" {...field} /></FormControl></FormItem>)} />
            <FormField control={form.control} name="host" render={({ field }) => (<FormItem><FormLabel>Host</FormLabel><FormControl><Input placeholder="smtp.example.com" {...field} /></FormControl></FormItem>)} />
            <FormField control={form.control} name="port" render={({ field }) => (<FormItem><FormLabel>Puerto</FormLabel><FormControl><Input type="number" {...field} /></FormControl></FormItem>)} />
            <FormField control={form.control} name="user" render={({ field }) => (<FormItem><FormLabel>Usuario</FormLabel><FormControl><Input placeholder="apikey" {...field} /></FormControl></FormItem>)} />
            <FormField control={form.control} name="password" render={({ field }) => (<FormItem><FormLabel>Contraseña</FormLabel><FormControl><Input type="password" placeholder={editingConfig ? "Dejar en blanco para no cambiar" : "••••••••"} {...field} /></FormControl></FormItem>)} />
            <FormField control={form.control} name="secure" render={({ field }) => (<FormItem className="flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm"><FormLabel>Usar TLS/SSL (Secure)</FormLabel><FormControl><Switch checked={field.value} onCheckedChange={field.onChange} /></FormControl></FormItem>)} />
