...
import { Loader2, Send, LifeBuoy, User, MessageSquareText, Clock, AlertCircle, Shield } from 'lucide-react';
...
const messageSchema = z.object({
  ...
  is_internal_note: z.boolean(), // Definido como boolean, el default se maneja en useForm
});
...
interface UserSupportTicketDetailDialogProps {
  open: boolean;
...
export function UserSupportTicketDetailDialog({ open, onOpenChange, ticketId, onTicketUpdated, userId }: UserSupportTicketDetailDialogProps) {
  ...
  const fetchTicketDetails = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`/api/users/${currentUserId}/support-tickets/${ticketId}`);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[800px] h-[90vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Detalles y conversación de tu ticket de soporte.
          </DialogDescription>
        </DialogHeader>
        <div className="flex-1 py-4 overflow-hidden flex flex-col">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div className="space-y-2">
              <h4 className="text-sm font-semibold">Descripción Inicial:</h4>
              <div className="prose prose-sm dark:prose-invert max-w-none border rounded-md p-3 bg-muted/20">
            ...
            <ScrollArea className="flex-1 border rounded-md p-4 bg-muted/20" ref={messagesScrollAreaRef}>
              <div className="space-y-4">
                ...
                {ticket.messages.map(msg => (
                  <div key={msg.id} className={cn("flex items-start gap-3", msg.sender.id === currentUserId ? "justify-end" : "justify-start")}>
                    <div className={cn("flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center", msg.sender.id === currentUserId ? "bg-primary text-primary-foreground" : (msg.is_internal_note ? "bg-blue-500 text-white" : "bg-secondary text-secondary-foreground"))}>
                    ...
                    <div className={cn("rounded-lg p-3 max-w-[85%]", msg.sender.id === currentUserId ? "bg-primary text-primary-foreground" : (msg.is_internal_note ? "bg-blue-500 text-white" : "bg-muted"))}>
                      <p className="font-semibold">
                        {msg.sender.id === currentUserId ? 'Tú' : (msg.is_internal_note ? 'Nota Interna' : 'Soporte')}
                        {msg.sender.first_name && msg.sender.id !== currentUserId && ` de ${msg.sender.first_name} ${msg.sender.last_name}`}
                        {msg.sender.email && msg.sender.id !== currentUserId && ` (${msg.sender.email})`}
                      </p>
                      <div className="prose prose-sm dark:prose-invert max-w-none mt-1 text-sm">
                        <ReactMarkdown remarkPlugins={[remarkGfm]}>{msg.content}</ReactMarkdown>
                      </div>
                      <p className={cn("text-xs mt-2", msg.sender.id === currentUserId ? "text-primary-foreground/70" : (msg.is_internal_note ? "text-blue-100" : "text-muted-foreground"))}>
                ...
                <form onSubmit={messageForm.handleSubmit(handleSendMessage)} className="space-y-2">
                  <FormField control={messageForm.control} name="content" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Textarea placeholder="Escribe tu respuesta aquí..." {...field} disabled={isSendingMessage} rows={3} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                  {/* Users cannot send internal notes, only admins */}
                  {isAdminOrSuperAdmin && (
