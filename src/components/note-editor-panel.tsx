...
import { Save, Loader2, Eye, Code, Wand2, X, Settings } from 'lucide-react';
...
export function NoteEditorPanel({ noteId, onNoteUpdated }: NoteEditorPanelProps) {
  ...
  // State for note settings, managed within this component
          ...
          <Popover>
            ...
            <PopoverContent className="w-64 p-4 space-y-4">
              <div className="space-y-2">
                <h4 className="font-medium leading-none">Configuración de Nota</h4>
                <p className="text-sm text-muted-foreground">Ajustes específicos para este editor.</p>
              </div>
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <Label htmlFor="note-font-size">Tamaño de Fuente</Label>
                  <Input id="note-font-size" type="number" value={noteFontSize} onChange={(e) => setNoteFontSize(Number(e.target.value))} className="w-20" min={10} max={24} />
                </div>
                <div className="flex items-center justify-between">
                  <Label htmlFor="note-auto-save">Guardado Automático</Label>
                  <Switch id="note-auto-save" checked={noteAutoSave} onCheckedChange={setNoteAutoSave} />
                </div>
              </div>
            </PopoverContent>
          </Popover>
