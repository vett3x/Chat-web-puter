...
import { NoteAiChat, ChatMessage } from './note-ai-chat'; // Import ChatMessage type
...
interface Note {
  ...
  content: string | null;
  updated_at: string;
  chat_history: ChatMessage[] | null; // Add chat_history to the type
}
...
export function NoteEditorPanel({ noteId, onNoteUpdated, noteFontSize, noteAutoSave }: NoteEditorPanelProps) {
  ...
  const fetchNote = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from('notes')
      .select('id, title, content, updated_at, chat_history') // Select chat_history
      .eq('id', noteId)
